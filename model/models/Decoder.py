...
class CWindowAttention(nn.Module):
    ...
    def forward(self, x, y, mask=None):
        ...
        y = self.q(y).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
...
class CRSTB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.residual_group = CBasicLayer(dim=dim,
    ...
    def forward(self, x, y, x_size):
        return self.residual_group(x, y, x_size)
...
class SEF_decoder(nn.Module):
    ...
    def forward_features_Re(self, x, y): 
        x = self.lrelu(self.conv_first1_A(x))
        x = self.lrelu(self.conv_first2_A(x))       
        x_size = (x.shape[2], x.shape[3])
        x = self.patch_embed(x)
    ...
    def forward(self, A, B):
        ...
        x = self.forward_features_Re(x, y)                  
