...
class Cross_WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.illu_fea = nn.Linear(dim, dim, bias=qkv_bias)
    ...
    def forward(self, x, y, z, mask=None):
        ...
        y = self.illu_fea(y).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)# 4.23凌晨添加
...
class Cross_WindowAttention_noreti(nn.Module):
    r""" Window based multi-head self attention (W-MSA) module with relative position bias.
    It supports both of shifted and non-shifted window.
...
class Cross_SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.norm1_illu = norm_layer(dim)
    ...
    def forward(self, x, y, z, x_size):
        ...
        y_max = torch.max(y, dim=2, keepdim=True).values
        y_min = torch.min(y, dim=2, keepdim=True).values

        y_inf = y_max + y_min - y
        x = self.norm1_A(x)
        y_vis = y
        y_vis = self.norm1_illu(y_vis)
        y_inf = self.norm1_illu(y_inf)
        z = self.norm1_B(z)
        x = x.view(B, H, W, C)
        y_vis = y_vis.view(B, H, W, C)
        y_inf = y_inf.view(B, H, W, C)
        z = z.view(B, H, W, C)

        # cyclic shift
        if self.shift_size > 0:
            shifted_x = torch.roll(x, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
            shifted_y_vis = torch.roll(y_vis, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
            shifted_y_inf = torch.roll(y_inf, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
        ...
        x_windows = window_partition(shifted_x, self.window_size)  # nW*B, window_size, window_size, C
        x_windows = x_windows.view(-1, self.window_size * self.window_size, C)  # nW*B, window_size*window_size, C
        y_vis_windows = window_partition(shifted_y_vis, self.window_size)  # nW*B, window_size, window_size, C
        y_vis_windows = y_vis_windows.view(-1, self.window_size * self.window_size, C)  # nW*B, window_size*window_size, C
        y_inf_windows = window_partition(shifted_y_inf, self.window_size)  # nW*B, window_size, window_size, C
        y_inf_windows = y_inf_windows.view(-1, self.window_size * self.window_size, C)  # nW*B, window_size*window_size, C
        ...
        # W-MSA/SW-MSA (to be compatible for testing on images whose shapes are the multiple of window size
        if self.input_resolution == x_size:
            attn_windows_A = self.attn_A(x_windows, y_vis_windows, z_windows, mask=self.attn_mask)  # nW*B, window_size*window_size, C
            attn_windows_B = self.attn_B(z_windows, y_inf_windows, x_windows, mask=self.attn_mask)  # nW*B, window_size*window_size, C
        else:
            attn_windows_A = self.attn_A(x_windows, y_vis_windows, z_windows, mask=self.calculate_mask(x_size).to(x.device))
            attn_windows_B = self.attn_B(z_windows, y_inf_windows, x_windows, mask=self.calculate_mask(x_size).to(z.device))
...
class Cross_BasicLayer(nn.Module):
    ...
    def forward(self, x, y, z, x_size):
        for blk in self.blocks:
            if self.use_checkpoint:
                x, z = checkpoint.checkpoint(blk, x, y, z, x_size)
            else:
                x, z = blk(x, y, z, x_size)
        if self.downsample is not None:
            x = self.downsample(x)
            z = self.downsample(z)
        return x, z
...
class CRSTB(nn.Module):
    ...
    def forward(self, x, y, z, x_size):    
        self.residual_group(x, y, z, x_size)
        return x, z
...
class CGFM(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans_img_a=3, in_chans_img_b=3, out_chans=3,
    ...
    def forward(self, lit_img, fea, inf_img):
        x = lit_img
        y = fea
        z = inf_img
