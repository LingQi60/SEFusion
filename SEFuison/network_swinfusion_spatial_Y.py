...
class Cross_WindowAttention(nn.Module):
    ...
    def forward(self, x, z, mask=None):
        ...
        kv = self.kv(z).reshape(B_, N, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
...
class Cross_SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, z, x_size):
        ...
        # W-MSA/SW-MSA (to be compatible for testing on images whose shapes are the multiple of window size
        # if self.input_resolution == x_size:
        #     attn_windows_A = self.attn_A(x_windows, y_windows, z_windows, mask=self.attn_mask)  # nW*B, window_size*window_size, C
        #     attn_windows_B = self.attn_B(z_windows, y_windows, x_windows, mask=self.attn_mask)  # nW*B, window_size*window_size, C
        # else:
        #     attn_windows_A = self.attn_A(x_windows, y_windows, z_windows, mask=self.calculate_mask(x_size).to(x.device))
        #     attn_windows_B = self.attn_B(z_windows, y_windows, x_windows, mask=self.calculate_mask(x_size).to(z.device))

        if self.input_resolution == x_size:
            attn_windows_A = self.attn_A(x_windows, z_windows, mask=self.attn_mask)  # nW*B, window_size*window_size, C
            attn_windows_B = self.attn_B(z_windows, x_windows, mask=self.attn_mask)  # nW*B, window_size*window_size, C
        else:
            attn_windows_A = self.attn_A(x_windows, z_windows, mask=self.calculate_mask(x_size).to(x.device))
            attn_windows_B = self.attn_B(z_windows, x_windows, mask=self.calculate_mask(x_size).to(z.device))
        ...
        shifted_z = window_reverse(attn_windows_B, self.window_size, H, W)  # B H' W' C
        ...
        z = shortcut_B + self.drop_path_B(z)
        z = z + self.drop_path_B(self.mlp_B(self.norm2_B(z)))
        return x, z
...
class Cross_BasicLayer(nn.Module):
    ...
    def forward(self, x, z, x_size):
        for blk in self.blocks:
            if self.use_checkpoint:
                x, z = checkpoint.checkpoint(blk, x, z, x_size)
            else:
                x, z = blk(x, z, x_size)
        if self.downsample is not None:
            x = self.downsample(x)
...
class CRSTB(nn.Module):
    ...
    def forward(self, x, z, x_size):
        ...
        #x1 = x
        #y1 = y
        x, z = self.residual_group(x, z, x_size)
...
class swinfusion_spatial(nn.Module):
    r""" SwinIR
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans_img_a=1, in_chans_img_b=1, out_chans=1,  # 可见光为Y通道，红外一个通道，输出也是一个通道
                 ...
                 seq_length=2,# 4.18下午 2->1
                 **kwargs):
        super(swinfusion_spatial, self).__init__()
        if ((in_chans_img_a % seq_length)):
            self.seq_length = 1
        else :
        ...
        embed_dim_temp = int(embed_dim / self.seq_length)
        print('in_chans: ', in_chans_img_a)
        if in_chans_img_a == 3 or in_chans_img_a == 6:
        ...
        # 他这个AB是什么情况
        self.conv_first1_A = nn.Conv2d(int(in_chans_img_a / self.seq_length), int(embed_dim_temp / self.seq_length), 3, 1, 1)
        self.conv_first1_B = nn.Conv2d(int(in_chans_img_b / self.seq_length), int(embed_dim_temp / self.seq_length), 3, 1, 1)
        self.conv_first2_A = nn.Conv2d(int(embed_dim_temp / self.seq_length), embed_dim_temp, 3, 1, 1)
        self.conv_first2_B = nn.Conv2d(int(embed_dim_temp / self.seq_length), embed_dim_temp, 3, 1, 1)
        self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=True)# 4.18下午0.2->0.1
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.conv_last_A = nn.Conv2d(embed_dim, num_out_ch, 3, 1, 1)
            # self.conv_last1_A = nn.Conv2d(embed_dim, embed_dim_temp, 3, 1, 1)
            # self.conv_last2_A = nn.Conv2d(embed_dim_temp, int(embed_dim_temp/2), 3, 1, 1)
            # self.conv_last3_A = nn.Conv2d(int(embed_dim_temp/2), num_out_ch, 3, 1, 1)
            self.conv_last_B = nn.Conv2d(embed_dim, num_out_ch, 3, 1, 1)
            # self.conv_last1_B = nn.Conv2d(embed_dim, embed_dim_temp, 3, 1, 1)
            # self.conv_last2_B = nn.Conv2d(embed_dim_temp, int(embed_dim_temp/2), 3, 1, 1)
            # self.conv_last3_B = nn.Conv2d(int(embed_dim_temp/2), num_out_ch, 3, 1, 1)
    ...
    # channel cross embedding
    def forward_features_Ex_A(self, x, x_size):
        x = rearrange(x, 'b (l c) h w -> b c (l h) w', l=self.seq_length)

        x = self.lrelu(self.conv_first1_A(x))
        x = self.lrelu(self.conv_first2_A(x))

        x = rearrange(x, 'b c (l h) w -> b (l c) h w', l=self.seq_length)          
        x = self.patch_embed(x)
        if self.ape:
            x = x + self.absolute_pos_embeds
        x = self.pos_drop(x)
    ...
    def forward_features_Ex_B(self, x, x_size):
        x = rearrange(x, 'b (l c) h w -> b c (l h) w', l=self.seq_length) 
    ...
    # def forward_features_Con(self, x, y, z, x_size):
    def forward_features_Con(self, x, z, x_size):
        x = self.patch_embed(x)
        # y是否要归一化？
        ...
        #     x, z = layer(x, y, z, x_size)

        for layer in self.layers_Fusion:
            x, z = layer(x, z, x_size)

        x = self.norm_Fusion_A(x)  # B L C
        x = self.patch_unembed(x, x_size)

        z = self.norm_Fusion_B(z)  # B L C
        z = self.patch_unembed(z, x_size)
        
        return x, z

    def forward_features_Re_A(self, x):        

        # x = self.lrelu(self.conv_last1_A(x))
        x = self.lrelu(self.conv_last_A(x))
        # x = self.conv_last_A(x) 

        return x
    
    def forward_features_Re_B(self, x):

        # x = self.lrelu(self.conv_last1_B(x))
        x = self.lrelu(self.conv_last_B(x))
    ...
    #     # Feedforward
    #     x = self.forward_features_Ex_A(x, x_size)
    #     z = self.forward_features_Ex_B(z, z_size)
    #     x, z = self.forward_features_Con(x, y, z, x_size)
    #     x = self.forward_features_Re_A(x)
    #     z = self.forward_features_Re_B(z)                 

    #     return x[:, :, :H*self.upscale, :W*self.upscale], z[:, :, :H*self.upscale, :W*self.upscale]
    ...
    def forward(self, lit_img, inf_img):
        ...
        z = self.forward_features_Ex_B(z, z_size)
        # x, z = self.forward_features_Con(x, y, z, x_size)
        x, z = self.forward_features_Con(x, z, x_size)
        x = self.forward_features_Re_A(x)
        z = self.forward_features_Re_B(z)                 
...
if __name__ == '__main__':
    ...
    height = 128
    width = 128
    model = swinfusion_spatial(upscale=2, img_size=(height, width),
                   window_size=window_size, img_range=1., depths=[6, 6, 6, 6],
                   embed_dim=60, num_heads=[6, 6, 6, 6], mlp_ratio=2, upsampler='',seq_length=2)
    print(model)
    # print(height, width, model.flops() / 1e9)

    x = torch.randn((1, 1, height, width))
    # y = torch.randn((1, 60, height, width))
    z = torch.randn((1, 1, height, width))
    x, z = model(x,z)
    print(x)
    print(z)
