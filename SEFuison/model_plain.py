...
class ModelPlain(ModelBase):
    ...
    def define_loss(self):
        ...
        if G_lossfn_type == 'l1':
        ...
        elif G_lossfn_type == 'loe':
            from models.loss_loe import loe_loss            
            self.G_lossfn = loe_loss().to(self.device)
        elif G_lossfn_type == 'mef':
            from models.loss_mef import fusion_loss_mef
            self.G_lossfn = fusion_loss_mef().to(self.device)
            ...
            self.G_lossfn = fusion_loss_mff().to(self.device)
        elif G_lossfn_type == 'nir':            
            from models.loss_nir import fusion_loss_nir
            self.G_lossfn = fusion_loss_nir().to(self.device)
    ...
    # ----------------------------------------
    def define_scheduler(self): # 加载学习率列表
        if self.opt_train['G_scheduler_type'] == 'CosineAnnealingLR':
            self.schedulers.append(lr_scheduler.CosineAnnealingLR(self.G_optimizer,self.opt_train['G_scheduler_t_max'],
    ...
    def netG_forward(self, phase='test'):
        self.img_vi_Y, self.img_ir, self.Y_E, self.E = self.netG(self.A, self.B)
        # with torch.no_grad():
    ...
    def optimize_parameters(self, current_step):
        ...
        if G_lossfn_type in ['loe', 'gt']:
            ...
            # total_loss, loss_text, loss_int, loss_ssim, loss_color = self.G_lossfn(self.A, self.B, self.E)
            total_loss, gradient_loss, contrast_loss, color_angle_loss, l1_loss = self.G_lossfn(self.img_vi_Y, self.img_ir, self.Y_E, self.E, self.A)
            # total_loss, gradient_loss, contrast_loss, l1_loss = self.G_lossfn(self.img_vi_Y, self.img_ir, self.Y_E, self.E, self.A)
        ...
        if G_lossfn_type in ['loe', 'mef', 'vif', 'mff', 'gt', 'nir', 'med']:
            self.log_dict['gradient_loss'] = gradient_loss.item()
            self.log_dict['contrast_loss'] = contrast_loss.item()
            self.log_dict['color_angle_loss'] = color_angle_loss.item()
            self.log_dict['l1_loss'] = l1_loss.item()
            # self.log_dict['Exp_loss'] = loss_exp.item()
